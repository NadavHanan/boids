...
lb = [boid(win_size*.5+rnd.randint(-50,50), win_size*.5+rnd.randint(-50,50), rnd.uniform(0.,math.pi*2)) for i in range(20)]
s = sqr(250, 250)
# mainloop
t = 0
run = True
while run:
    ...
    s.x = mean([lb[i].x for i in range(len(lb))])
    s.y = mean([lb[i].y for i in range(len(lb))])
    ...
    for i in range(len(lb)):
        lb[i].draw(win)
        lb[i].move()
        xa, ya = lb[i].avoide(lb, i)
        xw, yw = lb[i].inside(win_size)
        xm, ym = lb[i].match(lb, i)
        x = [xa, xw, xm, s.x]
        y = [ya, yw, ym, s.y]
